/*!
 * HPP Website SDK — Drop-in browser gate for the Human Presence Protocol.
 *
 * Exposes a single global: `window.HPP`.
 *
 *   HPP.gate({
 *     container: '#hpp-gate',                              // selector | HTMLElement
 *     verifier:  'https://hpp-verifier.onrender.com',      // verifier base URL
 *     site:      'example.com',                            // optional; defaults to verifier hostname
 *     sessionDuration: 600,                                // optional, seconds
 *     pollIntervalMs:  2000,                               // optional
 *     onUnlock:  (session) => { ... },                     // session = { sessionToken }
 *     onExpired: () => { ... },                            // optional
 ...
 *
 * License: Apache 2.0
 * Patents: USPTO Customer No. 224891 (see PATENT-NOTICE.md)
 */
(function (root, factory) {
  ...
  function siteFromVerifier(verifierUrl) {
    try {
      return new URL(verifierUrl).hostname;
    } catch (e) {
      return '';
    }
  }
  ...
  GateController.prototype._mountShell = function () {
    ...
    style.textContent = [
      ...
      '  padding: 2.5rem 1.5rem; border: 1px solid #ccc; border-radius: 6px;',
      ...
      '.hpp-qr    { width: 240px; height: 240px; display: block; margin: 0 auto 1.25rem; background: #fff; border: 1px solid #e8e8e8; }',
      '.hpp-qr.is-loading { opacity: .25; }',
      '.hpp-status {',
      '  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;',
      '  font-size: .78rem; letter-spacing: .04em; color: #777;',
  ...
  GateController.prototype._startPolling = function () {
    ...
    this.timer = setInterval(function () {
      ...
      fetch(self.opts.verifier + '/relay/' + encodeURIComponent(self.relayId))
        ...
        .then(function (data) {
          ...
          if (data && data.status === 'ready' && data.session_token) {
            ...
            try {
              ...
              // user-supplied callback errors should not break the SDK
        ...
        .catch(function () {
          // transient network errors are silent — the next tick retries.
        });
